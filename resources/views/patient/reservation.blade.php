...
<body>
 <!-- ///////////////////////////////////head///////////////////////////////////////////////////////// -->
  ...
  <h2 class="text-center text-primary fs-1">APPOINTMENTS FREE</h2>
  ...
  <table class="table fs-4" style="margin-bottom:200px;">
    ...
    <tbody>
      ...
      <tr>
        <td>{{ $clinic->users->first_name }}&nbsp;{{ $clinic->users->last_name }}</td>
        <td>{{ $clinic->medical_specialty }}</td>
        <td>{{ $appointment->date_of_inspection }}</td>
        <td>{{ $appointment->start_inspection }}</td>
        <td>{{ $appointment->end_inspection }}</td>
        <td>{{ $appointment->status }}</td>
        <td><a href="{{ route('reservation_date',[$appointment->id,$clinic->id]) }}"><button class="bg-warning" style="border-radius: 5px;width: 85px;">reserve</button></a></td>
      </tr>
      @endforeach
    </tbody>
  </table>
